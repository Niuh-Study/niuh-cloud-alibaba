...
public class NiuhUserDetailService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String userName) throws UsernameNotFoundException {
        ...
        NiuhUser niuhUser = new NiuhUser(sysUser.getUsername(),passwordEncoder.encode(sysUser.getPassword()),authorityList);
        niuhUser.setUserId(sysUser.getId());
        niuhUser.setNickName(sysUser.getNickname());
        niuhUser.setEmail(sysUser.getEmail());
        log.info("用户登陆成功:{}", JSON.toJSONString(niuhUser));
        return niuhUser;
    }
}
